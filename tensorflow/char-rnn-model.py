...
grad_var = [(tf.clip_by_value(grad, -5.0, 5.0), var) for grad, var in grad_var]
...
with tf.Session() as sess:
    ...
    while n <= max_steps:
        ...
        if n % print_step == 0:
            ...
            sample_indeces = [
                char_to_index[ch] for ch in data[start_index:start_index + sequence_length]
                ]
            h_prev_sample = np.copy(h_prev_)
            for t in range(200):
                sample_x = np_to_tf_one_hot(sample_indeces)
